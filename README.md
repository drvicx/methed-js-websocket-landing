...
08: 20220725_1600:
<pre>
- реализация логики заполнения блока с курсами валют данными приходящими из внешнего WebSocket Сервиса/Приложения;
- реализован модуль "exchange-rates.js" который: подключается к тестовому WebSocket веб-приложению (Сервису)
  транслирующему фиктивные данные о курсах валют каждые 5 секунд (ws://web-socket-current.herokuapp.com);
  формирует динамические li-элементы и выводит их в блок "Курсы валют" на основную страницу;

- В РЕЗУЛЬТАТЕ на странице "index.html" в разделе "Курсы валют" динамически в режиме реального времени с интервалом в 5 сек.
  выводятся фиктивные данные о курсах валют получаемых по протоколу WebSocket от Сервиса (NodeJS веб-приложения)
  размещенного на облачном VPS Heroku (сервис не является частью текущего проекта);
  данные в блок выводятся сверху-вниз, при этом старые элементы после 4о удаляются;
</pre>
...
- v20220725_1600 (08) -- Данные приходящие от WebSocket Сервиса и Блок с Курсами валют<br>
![preview](_preview/app-preview_20220725_1600.png?raw=true)
<br><br>
